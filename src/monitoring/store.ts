...
export async function getFromStore(env: StoreEnv, key: string): Promise<string | null> {
  const stmt = env.UPTIMEFLARE_D1.prepare('SELECT value FROM monitor_state WHERE key = ?')
...
export class CompactedMonitorStateWrapper {
  ...
  uncompact(): MonitorState {
    ...
    const hex2Uint8Arr = (hex: string): Uint8Array => {
      if (Uint8Array.fromHex) return Uint8Array.fromHex(hex)
  ...
  deleteMonitorState(monitorId: string) {
